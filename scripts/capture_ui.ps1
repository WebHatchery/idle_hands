...
.DESCRIPTION
    ...
    (IDLE_HANDS_CAPTURE_*) provided by macroquad_toolkit::capture in
    src/main.rs. The Phase 0 harness has a single boot state,
...
& $shared -GameDir $gameDir -Prefix "IDLE_HANDS" -Scenes $Scenes -Frames $Frames -OutputDir $OutputDir -SkipBuild:$SkipBuild
