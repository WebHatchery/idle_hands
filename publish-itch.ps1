...
try {
    if ($needsHtml5Package) {
        ...
        $demoWasm = Join-Path $PSScriptRoot "target-demo\wasm32-unknown-unknown\release\idle_hands.wasm"
        ...
        Copy-Item -LiteralPath $demoWasm -Destination (Join-Path $demoDir "idle_hands.wasm") -Force
