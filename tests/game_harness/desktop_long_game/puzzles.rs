...
use idle_hands::testing::games::game_2048::Game2048;
use idle_hands::testing::games::hangman::Hangman;
use idle_hands::testing::games::lights_out::LightsOut;
use idle_hands::testing::games::mastermind::Mastermind;
use idle_hands::testing::games::memory_pairs::MemoryPairs;
use idle_hands::testing::games::minesweeper::Minesweeper;
use idle_hands::testing::games::misc_games::{MiscGame, MiscKind};
use idle_hands::testing::games::nonogram::Nonogram;
use idle_hands::testing::games::sliding_puzzle::SlidingPuzzle;
use idle_hands::testing::games::sudoku::Sudoku;
use idle_hands::testing::games::word_grid::WordGrid;
use idle_hands::testing::games::word_ladder::WordLadder;
use idle_hands::testing::games::word_search::WordSearch;
...
fn sudoku() {
    let mut game = Sudoku::with_difficulty_and_seed(
        idle_hands::testing::games::sudoku::SudokuDifficulty::Medium,
        SESSION_SEED,
    );
...
fn nonogram() {
    let mut game = Nonogram::new_with_seed(
        idle_hands::testing::games::nonogram::NonogramPreset::Medium,
        SESSION_SEED,
    );
    let mut marks = 0;
    for round in 0..5 {
...
fn fivefold() {
    let mut game = idle_hands::testing::games::fivefold::Fivefold::new(SESSION_SEED);
    let mut rounds = 0;
    for round in 0..16 {
        for _ in 0..3 {
            assert!(game.roll());
        ...
        let category = game
            .hint_category()
            .unwrap_or(idle_hands::testing::games::fivefold::Category::Chance);
        assert!(game.choose_category(category));
        rounds += 1;
        game = idle_hands::testing::games::fivefold::Fivefold::new(SESSION_SEED + round + 1);
    }
    assert_eq!(rounds, 16);
    assert_serializable(&game);
}
