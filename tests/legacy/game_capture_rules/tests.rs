...
use idle_hands::testing::modules::game::game_capture_rules::*;
...
fn word_capture_keeps_the_first_filtered_rule_launchable_by_canonical_index() {
    ...
    let first =
        idle_hands::testing::rules_data::page_rows(state.rules_filter, state.library_scroll, 8)
            .into_iter()
            .next()
            .expect("word shelf has a first rule");

    assert_eq!(first.game, idle_hands::testing::state::GameId::Mastermind);
