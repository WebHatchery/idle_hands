...
fn assert_layout_is_clear(width: f32, height: f32) {
    ...
    assert_controls_are_clear(layout.board, &controls, width, height);
}

fn assert_controls_are_clear(board: Rect, controls: &[Rect], width: f32, height: f32) {
    assert!(controls.iter().all(|control| !control.overlaps(&board)));
