...
fn draw_home(state: &AppState, loaded: usize) {
    ...
    text(
        ...
        185.,
        73.,
        crate::accessibility::text_size(8., state.large_text),
        crate::theme::BRASS,
    );
    text(
        selected.title(),
...
fn draw_library(state: &AppState) {
    ...
    panel(Rect::new(690., 4., 135., 58.), crate::theme::PAPER_LIGHT);
    text(
        &format!(
            "SORT {}",
...
fn library_clicks(state: &AppState, p: Vec2) -> Vec<UiAction> {
    ...
    if crate::ui::hit(Rect::new(690., 4., 135., 58.), p) {
        return vec![UiAction::CabinetSort];
    }
    for (index, game) in page_games(state).iter().copied().enumerate() {
        let rect = game_rect(index);
        if crate::ui::hit(Rect::new(rect.right() - 84., rect.y, 40., rect.h), p) {
