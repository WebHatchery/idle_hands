...
fn draw_home(state: &AppState, _loaded: usize) {
    ...
    text(
        ...
        24.,
        77.,
        crate::accessibility::text_size(9., state.large_text),
        crate::theme::BRASS,
    );
...
fn draw_library(state: &AppState) {
    ...
    panel(Rect::new(190., 10., 104., 40.), crate::theme::PAPER_LIGHT);
    text(
        &format!(
            "SORT {}",
...
fn library_clicks(state: &AppState, p: Vec2) -> Vec<UiAction> {
    ...
    if crate::ui::hit(Rect::new(190., 10., 104., 40.), p) {
        return vec![UiAction::CabinetSort];
    }
    for (index, game) in page_games(state).iter().copied().enumerate() {
        let rect = game_rect(index);
        if crate::ui::hit(Rect::new(rect.right() - 88., rect.y, 44., rect.h), p) {
    ...
    let page = crate::cabinet_data::page(state, crate::cabinet_data::PORTRAIT_PAGE_SIZE);
    if page.has_previous() && crate::ui::hit(Rect::new(10., 632., 100., 46.), p) {
        return vec![UiAction::CabinetScroll(
            -(crate::cabinet_data::PORTRAIT_PAGE_SIZE as i8),
        )];
    }
    if page.has_next() && crate::ui::hit(Rect::new(250., 632., 100., 46.), p) {
        return vec![UiAction::CabinetScroll(
            crate::cabinet_data::PORTRAIT_PAGE_SIZE as i8,
        )];
    }
