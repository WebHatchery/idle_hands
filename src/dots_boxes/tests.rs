...
#[test]
fn drawn_edges_keep_the_owner_who_claimed_them() {
    let mut game = DotsBoxes::new(1);
    assert!(game.play(Edge::Horizontal(0)));

    assert_eq!(game.edge_owner(Edge::Horizontal(0)), 1);
    assert!(game
        .horizontal_owners
        .iter()
        .chain(game.vertical_owners.iter())
        .any(|owner| *owner == 2));
}

#[test]
fn hint_avoids_offering_a_third_edge_when_safe_lines_remain() {
    let mut game = DotsBoxes::new(1);
    game.horizontal[0] = true;
    game.horizontal[4] = true;

    assert_eq!(game.edge_risk(Edge::Vertical(0)), 1);
    let hint = game.hint_edge().unwrap();
    assert_eq!(game.edge_risk(hint), 0);
}
