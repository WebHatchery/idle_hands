...
impl Game {
    pub async fn new() -> Self {
        let data = GameData::load().unwrap_or_else(|err| {
            panic!("Idle Hands embedded data failed to load: {}", err);
        });
        ...
        notifications.info(format!(
            "Idle Hands Phase 0 harness booted; {} manifest textures loaded",
            loaded_assets
        ));
    ...
    fn apply_action(&mut self, action: UiAction) {
        match action {
            UiAction::NewGame => {
                ...
                self.notifications
                    .info("Started a fresh Phase 0 harness session");
            }
    ...
    fn delete_save(&mut self) {
        match delete_slot(&self.data.config.game_name, &self.data.config.save_slot) {
            Ok(()) => {
                self.notifications
                    .info("Deleted Idle Hands harness save slot");
                self.refresh_save_state();
            }
